...
// on-chain, as this in-memory map won't persist across requests.
...
export const { ThirdwebAuthHandler, getUser } = ThirdwebAuth({
  ...
  callbacks: {
    onLogin: async (address: string, req?: NextApiRequest | undefined) => {
      // Here we can run side-effects like creating and updating user data
      // whenever a user logs in.

      const existingUser = await User.findOne({ address });

      // Create a new user entry if the user does not exist
      if (!existingUser) {
